...
<section class="cate_show col-md">
    ...
    <div class="shadow_content">
        <h5 class="cate_lable" > <i class="fas fa-envelope"></i> الرسائل المرسلة</h5>
        <div class="cate_search">
            <form action="/email/search_sendedemail" method="POST">
                {{ csrf_field() }}
                {{method_field('GET')}}
                <div class="msg2">
                    @if (isset($msg))
        ...
        <table class="table table-bordered">
            ...
            @foreach ($project as $item)
                <tr>
                    ...
                    <td>
                    <form action="/email/deleted1?email_id={{$item->email_id}}" method="POST" class="table_action">
                        {{ csrf_field() }}
                        {{ method_field('DELETE')}}
