    ...
    <div class="row "> 
        ...
        <div class="col-md-3 gal_show ">
              ...
              <div class="row gal_show_icons">
                    <form action="/gallery/updateProductsPhotos/update?media_id={{$item->media_id}}" method="POST"   class="table_action">
                        {{ csrf_field() }}
                        {{ method_field('GET')}}
                        <button type="submit" class="btn  table_action_btn"> <i class="fa fa-refresh" aria-hidden="true"></i></button>
                    </form>
                    <form action="/gallery/updateProductsPhotos/delete?media_id={{$item->media_id}}" method="POST" class="table_action">
                        {{ csrf_field() }}
                        {{ method_field('GET')}}
