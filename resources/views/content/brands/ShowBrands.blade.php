...
<section class="brand_show col-md">
    <div class="shadow_content">
        <h5 class="cate_lable" > <i class="fas fa-copyright"></i> عرض الوكالات</h5>
        <div class="cate_search">
            <form action="/brandSearch/avaliableBrands" method="POST">
                {{ csrf_field() }}
                {{method_field('GET')}}
                <div class="msg">
                    @if (isset($msg))
        ...
        <table class="table table-bordered">
            ...
            <tr>
                ...
                <td>
                    <form action="/updatebrands?brand_id={{$item->brand_id}}" method="POST" class="table_action">
                        {{ csrf_field() }}
                        {{ method_field('GET')}}
                        <button type="submit" class="btn table_action_btn"> <i class="fa fa-refresh" aria-hidden="true"></i></button>
                    </form>
                    <form action="/updatebrands/delete?brand_id={{$item->brand_id}}" method="POST" class="table_action">
                        {{ csrf_field() }}
                        {{ method_field('DELETE')}}
